                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            aria-label={t('traffic_search_placeholder')}
                        />
                    </div>
                    ...
                    <div className="flex bg-slate-900/50 rounded-xl border border-slate-700 p-1">
                        <button
                            ...
                            aria-pressed={groupBy === 'speed_level'}
                            aria-label={t('traffic_group_speed')}
                        >
                        ...
                        <button
                            ...
                            aria-pressed={groupBy === 'district'}
                            aria-label={t('traffic_group_district')}
                        >
                    ...
                    <div key={groupName} className="mb-2">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-800/80 sticky top-0 z-20 backdrop-blur-md">
                                <tr>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('segment_id')} aria-sort={sortField === 'segment_id' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                        <div className="flex items-center gap-1">ID <SortIcon field="segment_id" /></div>
                                    </th>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('district')} aria-sort={sortField === 'district' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                        <div className="flex items-center gap-1">{t('traffic_district')} <SortIcon field="district" /></div>
                                    </th>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('v85')} aria-sort={sortField === 'v85' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                    ...
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('car')} aria-sort={sortField === 'car' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                        <div className="flex items-center gap-1"><Car className="w-3 h-3" /> {t('traffic_cars')} <SortIcon field="car" /></div>
                                    </th>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('bike')} aria-sort={sortField === 'bike' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                        <div className="flex items-center gap-1"><Bike className="w-3 h-3" /> {t('traffic_bikes')} <SortIcon field="bike" /></div>
                                    </th>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('pedestrian')} aria-sort={sortField === 'pedestrian' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
                                        <div className="flex items-center gap-1"><Footprints className="w-3 h-3" /> {t('traffic_peds')} <SortIcon field="pedestrian" /></div>
                                    </th>
                                    <th className="p-4 text-xs font-medium text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('heavy')} aria-sort={sortField === 'heavy' ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} role="columnheader">
