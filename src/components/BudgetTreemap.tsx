...
import type { TreeNode } from '@/lib/budget-tree';

type TreemapNode = d3.HierarchyRectangularNode<TreeNode> & { leafId?: string; clipId?: string };
...
export default function BudgetTreemap({ data, onDrillDown, isRoot }: BudgetTreemapProps) {
    ...
    useEffect(() => {
        ...
        const nodes = (root.children || [root]) as TreemapNode[];
        ...
        leaf.append('rect')
            .attr('id', (d) => (d.leafId = `leaf-${Math.random().toString(36).substring(2, 11)}`))
        ...
        leaf.append('clipPath')
            .attr('id', (d) => (d.clipId = `clip-${Math.random().toString(36).substring(2, 11)}`))
            .append('use')
